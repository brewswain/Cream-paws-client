...
const ItemizedBreakdownCard = ({ mode }: ItemizedBreakdownCardProps) => {
   ...
   const mappedVatArray = orders
      ...
      .map(
         (order) =>
            order.chow_details.flavours.varieties.wholesale_price * 0.125
      );
                  ...
                  <Checkbox.Group onChange={setGroupValues} value={groupValues}>
                     {courierOrders.length > 0
                        ? courierOrders.map((order, index) => {
                             ...
                             const orderIdArray = order.orders.map(
                                (o) => o.order_id
                             );
                             ...
                             const calculatedFees = order.orders
                                .map(
                                   (o) =>
                                      (o.chow_details.flavours.varieties
                                         .retail_price -
                                         o.chow_details.flavours.varieties
                                            .wholesale_price) *
                                      0.5 *
                     ...
                     {formattedOrders.length > 0 ? (
                        formattedOrders.map((order, index) => {
                           ...
                           const vatExclusivePrice: string = Dinero({
                              amount: Math.round(
                                 chosenPrice * order.quantity * 100
                              ),
                           }).toFormat('$0,0.00');
                                          ...
                                          <Text>
                                             {isWarehouseOrders
                                                ? order.chow_details.flavours
                                                     .varieties.wholesale_price
                                                : order.chow_details.flavours
                                                     .varieties.retail_price}
                                          </Text>
                                       </Text>
                                    </View>
               ...
               <View style={totalWrapper}>
                  <View style={priceWrapper}>
                     ...
                     <Text style={subTotalCost}>
                        {subTotal
                           ? Dinero({ amount: subTotal || 0 }).toFormat(
                                '$0,0.00'
                             )
                           : null}
                     </Text>
                  ...
                  <View style={priceWrapper}>
                     ...
                     <Text style={vatCost}>
                        {totalVat
                           ? Dinero({ amount: totalVat || 0 }).toFormat(
                                '$0,0.00'
                             )
                           : null}
                     </Text>
